...
export function PageDraftEditor({
  ...
  const [description, setDescription] = useState(
    cluster.suggestedDescription || cluster.description,
  );
  const [theme, setTheme] = useState<Theme>(
    cluster.suggestedTheme || cluster.theme || "family",
  );
  const [isEditing, setIsEditing] = useState(false);
  const [discardedPhotoIds, setDiscardedPhotoIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const backgroundUrl = cluster.backgroundPath
    ? getPhotoUrl(cluster.backgroundPath)
    : null;
          ...
          {isEditing ? (
            ...
            <>
              ...
              <div className="flex items-center flex-wrap gap-2 mb-6">
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium ${themeStyle.bg} ${themeStyle.accent}`}
                >
                  {themeStyle.label}
                </span>
